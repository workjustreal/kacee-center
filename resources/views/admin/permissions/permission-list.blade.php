@extends('layouts.master-layout', ['page_title' => "จัดการสิทธิ์การใช้งาน"])
@section('css')
<!-- third party css -->
    ...
    <div class="row">
        <div class="col-12">
            <div class="page-title-box">
                ...
                <h4 class="page-title">จัดการสิทธิ์การใช้งาน</h4>
            </div>
        </div>
    </div>
    <!-- end page title -->
                    ...
                    <div class="row justify-content-between mb-2">
                        ...
                        <div class="col-sm-6">
                            <div class="mb-2 text-sm-end">
                                <a href="{{url('/admin/permissions/create')}}" class="btn btn-primary waves-effect waves-light">
                                    <i class="mdi mdi-plus-circle me-1"></i> เพิ่มสิทธิ์การใช้งาน </a>
                            </div>
                        </div>
                    </div>
                    <table id="table" data-toggle="table" data-search="true" data-search-align="left" data-search-selector="#search" data-ajax="ajaxRequest" data-query-params="queryParams" class="table text-nowrap">
                        <thead>
                            <tr>
                                ...
                                <th data-field="permission" data-sortable="true">สิทธิ์การใช้งาน</th>
                                <th data-field="description" data-sortable="true">รายละเอียดสิทธิ์</th>
...
<script type="text/javascript">
    ...
    function ajaxRequest(params) {
        var url = "{{ url('admin/permissions/search') }}";
        $.get(url + '?' + $.param(params.data)).then(function (res) {
            params.success(res)
        });
    }
    function deletePermissionConfirmation(id) {
        Swal.fire({
            icon: "warning",
            title: "คุณต้องการลบสิทธิ์ ใช่ไหม?",
            ...
            preConfirm: () => {
                return fetch(`{{ url('admin/permissions/del') }}/` + id)
                    .then((response) => {
                        if (!response.ok) {
