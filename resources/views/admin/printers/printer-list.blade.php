@extends('layouts.master-layout', ['page_title' => "จัดการเครื่องพิมพ์"])
@section('css')
<!-- third party css -->
    ...
    <div class="row">
        <div class="col-12">
            <div class="page-title-box">
                ...
                <h4 class="page-title">จัดการเครื่องพิมพ์</h4>
            </div>
        </div>
    </div>
    <!-- end page title -->
                    ...
                    <div class="row justify-content-between mb-2">
                        ...
                        <div class="col-sm-6">
                            <div class="mb-2 text-sm-end">
                                <a href="{{url('/admin/printers/create')}}" class="btn btn-primary waves-effect waves-light">
                                    <i class="mdi mdi-plus-circle me-1"></i> เพิ่มเครื่องพิมพ์ </a>
                            </div>
                        </div>
                    </div>
                    <table id="table" data-toggle="table" data-search="true" data-search-align="left" data-search-selector="#search" data-ajax="ajaxRequest" data-query-params="queryParams" class="table text-nowrap">
                        <thead>
                            <tr>
                                ...
                                <th data-field="name" data-sortable="true">ชื่อเครื่องพิมพ์</th>
                                <th data-field="description" data-sortable="true">รายละเอียด</th>
                                <th data-field="type" data-sortable="true">ประเภท</th>
                                <th data-field="client_ip" data-sortable="true">ไอพี</th>
                                <th data-field="role" data-sortable="true">ผู้ใช้งาน</th>
...
<script type="text/javascript">
    ...
    function ajaxRequest(params) {
        var url = "{{ url('admin/printers/search') }}";
        $.get(url + '?' + $.param(params.data)).then(function (res) {
            params.success(res)
        });
    }
    function deletePrinterConfirmation(id) {
        Swal.fire({
            icon: "warning",
            ...
            preConfirm: () => {
                return fetch('/admin/printers/destroy', {
                        method: 'POST',
                        headers: {
