@extends('layouts.master-layout', ['page_title' => "ข้อมูลพนักงาน"])
@section('css')
<!-- third party css -->
        ...
        <div class="col-sm-12">
            <div class="card">
                <div class="card-body">
                    ...
                    <div id="exportEditForm" class="hidd"></div>
                    <div class="row justify-content-between">
                        ...
                        <div class="col-sm-8">
                            <div class="mb-2 text-sm-end">
                                ...
                                <a href="{{ url('organization/employees/upload') }}" class="btn btn-primary waves-effect waves-light">
                                    <i class="mdi mdi-upload me-1"></i> อัปโหลดข้อมูลพนักงาน </a>
                                <a href="{{ url('organization/employees/create') }}" class="btn btn-primary waves-effect waves-light">
                                    <i class="mdi mdi-plus-circle me-1"></i> เพิ่มข้อมูลพนักงาน </a>
                                <button type="button" class="btn btn-soft-secondary waves-effect waves-light" onclick="exportExcel();">Export</button>
                                <button type="button" class="btn btn-soft-secondary waves-effect waves-light" onclick="exportEditExcel();">Export (แก้ไข)</button>
                                @else
                                <button type="button" class="btn btn-soft-secondary waves-effect waves-light" onclick="exportExcel();">Export</button>
                                @endif
                            </div>
                    ...
                    <table id="table" data-toggle="table" data-loading-template="loadingTemplate" data-undefined-text="" data-ajax="ajaxRequest" data-query-params="queryParams" data-search="true"
...
<script type="text/javascript">
    ...
    function exportExcel() {
        var url = '{{ url("organization/employees/export") }}';
        $("#exportForm").append('<form action="' + url + '" method="GET">');
        $("#exportForm form").append(
            '<input type="text" name="level0" value="' + $("#level0").val() + '"/>'
        );
        $("#exportForm form").append(
            '<input type="text" name="level1" value="' + $("#level1").val() + '"/>'
        );
        $("#exportForm form").append(
            '<input type="text" name="level2" value="' + $("#level2").val() + '"/>'
        );
        $("#exportForm form").append(
            '<input type="text" name="level3" value="' + $("#level3").val() + '"/>'
        );
        $("#exportForm form").append(
            '<input type="text" name="level4" value="' + $("#level4").val() + '"/>'
        );
        $("#exportForm form").append(
        ...
        $("#exportForm form").append(
            '<input type="text" name="emp_type" value="' +
                $("#emp_type").val() +
                '"/>'
        );
        $("#exportForm form").append(
            '<input type="text" name="emp_status" value="' +
                $("#emp_status").val() +
                '"/>'
        );
    ...
    function exportEditExcel() {
        var url = '{{ url("organization/employees/export-edit") }}';
        $("#exportEditForm").append('<form action="' + url + '" method="GET">');
        $("#exportEditForm form").append(
            '<input type="text" name="level0" value="' + $("#level0").val() + '"/>'
        );
        $("#exportEditForm form").append(
            '<input type="text" name="level1" value="' + $("#level1").val() + '"/>'
        ...
        $("#exportEditForm form").submit();
        $("#exportEditForm").html("");
    }
    function deleteEmployeeConfirmation(emp_id) {
        Swal.fire({
            icon: "warning",
            ...
            preConfirm: () => {
                return fetch(`/organization/employees/del/` + emp_id)
                    .then((response) => {
                        if (!response.ok) {
