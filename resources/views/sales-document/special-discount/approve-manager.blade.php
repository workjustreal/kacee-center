...
@section('content')
    <div class="container-fluid">
        ...
        <div class="card">
            ...
            <div class="card-body">
                <form method="get" action="{{ route('special.manger.approve') }}">
                    {{ csrf_field() }}
                    <div class="row">
                ...
                <table data-toggle="table" data-search="true" data-show-columns="false" data-page-list="[5, 10, 20]"
                    ...
                    <tbody>
                        @foreach ($special as $spc)
                            @if ($spc->doc_status != 2)
                                @php
                                    $log_list = json_decode($spc->log_list);
                                    $count = 0;
                                @endphp
                                <tr>
                                    <td>
                                        <a class="btn btn-outline-primary waves-effect waves-light"
                                            href="{{ url('sales-document/special-discount/preview/view') }}/{{ $spc->doc_id }}/{{ $spc->doc_status }}">
                                            {{ $spc->doc_id }}
                                        </a>
                                    </td>
                                    <td><i class="fe-clock"></i>
                                        {{ $thaiDateHelper->shortDateFormat($spc->created_at) }}</td>
                                    @php
                                        $name_cres = Auth::User()->findEmployee($spc->emp_id);
                                    ...
                                    <td>{{ $spc->customer_code }}</td>
                                    <td><i class="mdi mdi-storefront-outline"></i> {{ $spc->customer_name }}</td>
                                    <td>
                                        @php
                                            if ($spc->sec_approve != null) {
                                                $last = $spc->sec_approve;
                                            } elseif ($spc->mn_approve != null) {
                                                $last = $spc->mn_approve;
                                            } else {
                                                $last = '';
                                            }
                                    ...
                                    <td>
                                        @if ($spc->doc_status == 9)
                                            <!-- Modal -->
                                            <div class="modal fade" id="historyPrint{{ $spc->id }}" tabindex="-1"
                                            ...
                                            <a href="javascript:void(0)" data-bs-toggle="modal"
                                                data-bs-target="#historyPrint{{ $spc->id }}">
                                    ...
                                    <td>
                                        ...
                                        @if ($spc->doc_status == 9)
                                            <button data-bs-toggle="modal" data-bs-target="#printModal"
                                                value="{{ $spc->doc_id }}" title="Print" class="btnPrint">
