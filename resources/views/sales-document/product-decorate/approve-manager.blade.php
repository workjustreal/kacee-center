...
@section('content')
    <div class="container-fluid">
                            ...
                            <tr>
                                <td>
                                    <a class="btn btn-outline-primary waves-effect waves-light"
                                        href="{{ url('sales-document/product-decorate/preview/view') }}/{{ $wa->doc_id }}/{{ $wa->doc_status }}">
                                        {{ $wa->doc_id }}
                                    </a>
                                </td>
                                <td><i class="fe-clock"></i> {{ $thaiDateHelper->shortDateFormat($wa->created_at) }}</td>
                                @php
        ...
        <div class="card">
            ...
            <div class="card-body">
                <form method="get" action="{{ route('decorate.manger.approve') }}">
                    {{ csrf_field() }}
                    <div class="row">
                        <div class="col-lg-3 mb-2">
                            <label for="doc_date" class="form-label mb-0">เดือน/ปี</label>
                            <div class="form-group">
                                <input type="text" class="form-control month-datepicker" id="doc_date" name="doc_date"
                                    placeholder="เลือกเดือน" value="{{ old('doc_date') }}">
                            </div>
                        </div>
                        <div class="col-lg-3 mb-2">
                        ...
                        @foreach ($decorate as $decor)
                            @if ($decor->doc_status != 2)
                                ...
                                <tr>
                                    ...
                                    @php
                                        $name_cres = Auth::User()->findEmployee($decor->emp_id);
                                        if ($name_cres->nickname) {
                                            $emp_name = $name_cres->name . ' ' . $name_cres->surname . ' ( ' . $name_cres->nickname . ' )';
                                        } else {
                                            $emp_name = $name_cres->name . ' ' . $name_cres->surname;
                                        }
                                    @endphp
                                    <td><i class="fe-user"></i> {{ $emp_name }}</td>
                                    ...
                                    <td>
                                        @if ($decor->doc_status == 9)
                                            <!-- Modal -->
                                            <div class="modal fade" id="historyPrint{{ $decor->id }}" tabindex="-1"
