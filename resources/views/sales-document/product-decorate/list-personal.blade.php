@extends('layouts.master-layout', ['page_title' => 'รายการขอสินค้าตกแต่งหน้าร้าน'])
@section('css')
    <!-- third party css -->
...
@section('content')
                ...
                <div class="page-title-box">
                    <div class="page-title-right">
                        <ol class="breadcrumb m-0">
                            ...
                            <li class="breadcrumb-item active">Product Decorate</li>
                        </ol>
                    </div>
                    <h4 class="page-title">รายการขอสินค้าตกแต่ง</h4>
                </div>
            </div>
        </div>
        <!-- end page title -->
        ...
        <div class="row">
            <div class="col-sm-12">
                <div class="card">
                    <div class="card-body">
                        <div class="row">
                            <form method="get" action="{{ route('decorate.list.personal') }}">
                                {{ csrf_field() }}
                                <div class="row">
                            ...
                            <div>
                                <a href="{{ url('/sales-document/product-decorate/request') }}" type="button"
                                    class="btn btn-primary float-end"><i class="fe-file-plus"></i>
                                    สร้างคำขอ</a>
                            </div>
                            <table data-toggle="table" data-search="true" data-show-columns="false"
                                ...
                                <thead class="table-light">
                                    <tr>
                                        ...
                                        <th data-field="shop-name">ขื่อร้าน</th>
                                        <th data-field="approve-name">อนุมัติล่าสุด</th>
                                ...
                                <tbody>
                                    @foreach ($decorate as $decor)
                                        @php
                                            $log_list = json_decode($decor->log_list);
                                            $count = 0;
                                        @endphp
                                        <tr>
                                            <td>
                                                <a class="btn btn-outline-primary waves-effect waves-light"
                                                    href="{{ url('sales-document/product-decorate/preview/view') }}/{{ $decor->doc_id }}/{{ $decor->doc_status }}">
                                                    {{ $decor->doc_id }}
                                                </a>
                                            </td>
                                            <td><i class="fe-clock"></i>
                                                {{ $thaiDateHelper->shortDateFormat($decor->created_at) }}</td>
                                            @php
                                                $name_cre = Auth::User()->findEmployee($decor->emp_id);
                                            ...
                                            <td>{{ $decor->customer_code }}</td>
                                            <td><i class="mdi mdi-storefront-outline"></i> {{ $decor->customer_name }}</td>
                                            <td>
                                                @php
                                                    if ($decor->sec_approve != null) {
                                                        $last = $decor->sec_approve;
                                                    } elseif ($decor->mn_approve != null) {
                                                        $last = $decor->mn_approve;
                                                    } else {
                                                        $last = '';
                                                    }
                                                    if ($last) {
                                                        $lastApprove = Auth::User()->findEmployee($last);
                                                        if ($lastApprove->nickname) {
                                                            $last_names = $lastApprove->name . ' ' . $lastApprove->surname . ' ( ' . $lastApprove->nickname . ' )';
                                                        } else {
                                                            $last_names = $lastApprove->name . ' ' . $lastApprove->surname;
                                                        }
                                                    } else {
                                                        $last_names = '';
                                                    }
                                            ...
                                            <td>
                                                @if ($decor->doc_status == 9)
                                                    <!-- Modal -->
                                                    <div class="modal fade" id="historyPrint{{ $decor->id }}"
                                                    ...
                                                    <a href="javascript:void(0)" data-bs-toggle="modal"
                                                        data-bs-target="#historyPrint{{ $decor->id }}">
                                            ...
                                            <td>
                                                ...
                                                @if ($decor->doc_status == 9)
                                                    <button data-bs-toggle="modal" data-bs-target="#printModal"
                                                        value="{{ $decor->doc_id }}" title="Print" class="btnPrint">
                                                        <i class="mdi mdi-printer text-blue me-1"></i>
                                                    </button>
                                                @endif
                                                @if (auth()->user()->emp_id == $decor->emp_id)
                                                    @if ($decor->doc_status == 2 || $decor->doc_status == 3 || $decor->doc_status == 0)
                                                        <a
                                                            href="{{ url('sales-document/product-decorate/edit') }}/{{ $decor->doc_id }}/{{ $decor->id }}/{{ $decor->doc_status }}">
                                                            <i class="fe-edit fs-5 text-info"
                                                                title="แก้ไขคำขอ"></i>&nbsp;&nbsp;
                                                        </a>
                                                    @endif
                                                    @if ($decor->doc_status == 2)
                                                        <a href="javascript: void(0);"
                                                            onclick="deleteRequest('{{ $decor->doc_id }}',{{ $decor->id }})">
                                                            <i class="fe-trash-2 fs-5 text-danger" title="ลบคำขอ"></i>
                                                        </a>
                                                    @endif
                                                @endif
                                                {{-- หัวหน้า --}}
                                                @if (Auth::User()->checkApproveMar() && $decor->doc_status == 1)
                                                    <a
                                                        href="{{ url('sales-document/product-decorate/edit') }}/{{ $decor->doc_id }}/{{ $decor->id }}/{{ $decor->doc_status }}">
                                                        <i class="fe-edit fs-5 text-info"
                                                            title="แก้ไขคำขอ"></i>&nbsp;&nbsp;
    ...
    <div id="printModal" class="modal fade" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1"
        ...
        <div class="modal-dialog modal-xl">
            <div class="modal-content">
                <div class="modal-header">
                    <h4 class="modal-title" id="printModalLabel">แบบฟอร์มขอสินค้าตกแต่งหน้าร้าน/สินค้าตัวอย่าง/แสตนโชว์
                        KACEE</h4>
...
@section('script')
    ...
    <script>
        ...
        $(document).on('click', '.btnPrint', function() {
            ...
            $("#pdf").attr("src",
                "{{ url('sales-document/product-decorate/print') }}/" + objData.toString());
            $.ajax({
                type: 'POST',
        ...
        function deleteRequest(doc_id, id) {
            Swal.fire({
                ...
                preConfirm: () => {
                    ...
                    return fetch('/sales-document/product-decorate/delete/', {
                            method: 'POST',
                            headers: {
