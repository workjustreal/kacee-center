@extends('layouts.master-layout', ['page_title' => "อนุมัติใบบันทึกวันทำงาน"])
@section('css')
<style>
            ...
            <div class="page-title-box">
                <div class="page-title-right">
                    <ol class="breadcrumb m-0">
                        ...
                        <li class="breadcrumb-item active">อนุมัติใบบันทึกวันทำงาน</li>
                    </ol>
                </div>
                <h4 class="page-title">อนุมัติใบบันทึกวันทำงาน</h4>
            </div>
        </div>
    </div>
    <!-- end page title -->
        ...
        <div class="col-lg-9">
            <div class="card ribbon-custom-box h-100">
                <div class="card-body">
                    ...
                    <div class="mt-4 mb-1">
                        <a href="{{ url('leave/approve/dashboard') }}" class="btn btn-secondary me-2"><i class="mdi mdi-keyboard-backspace me-1"></i> ย้อนกลับ</a>
                        <div class="text-end">
                            <button type="button" class="btn btn-primary waves-effect waves-light me-2" onclick="approveConfirmation('{{ $leave->id }}');"><i class="mdi mdi-check-bold me-1"></i> อนุมัติ</button>
                            <button type="button" class="btn btn-danger waves-effect waves-light" onclick="cancelConfirmation('{{ $leave->id }}');"><i class="mdi mdi-close-circle-outline me-1"></i> ไม่อนุมัติ</button>
                        </div>
                    </div>
                </div>
            </div>
...
<script type="text/javascript">
    function approveConfirmation(id) {
        Swal.fire({
            icon: "warning",
            title: "คุณต้องการอนุมัติใบบันทึกวันทำงาน ใช่ไหม?",
            ...
            preConfirm: () => {
                return fetch('/leave/approve/emp-record-working-approved', {
                        method: 'POST',
                        headers: {
            ...
            if (result.isConfirmed) {
                Swal.fire({
                    icon: "success",
                    title: "อนุมัติใบบันทึกวันทำงานเรียบร้อย!",
                });
                setTimeout(() => {
    ...
    function cancelConfirmation(id) {
        Swal.fire({
            icon: "warning",
            title: "คุณต้องการยกเลิกบันทึกวันทำงาน ใช่ไหม?",
            ...
            preConfirm: () => {
                ...
                return fetch('/leave/approve/emp-record-working-cancel', {
                        method: 'POST',
                        headers: {
                            'Content-type': 'application/json; charset=UTF-8',
                            'X-CSRF-TOKEN': '{{csrf_token()}}',
