@extends('layouts.master-layout', ['page_title' => "จำนวนวันหยุดประจำปี"])
@section('css')
<!-- third party css -->
            ...
            <div class="page-title-box">
                <div class="page-title-right">
                    <ol class="breadcrumb m-0">
                        ...
                        <li class="breadcrumb-item active">จำนวนวันหยุดประจำปี</li>
                    </ol>
                </div>
                <h4 class="page-title">จำนวนวันหยุดประจำปี</h4>
            </div>
        </div>
    </div>
    <!-- end page title -->
                    ...
                    <div class="row justify-content-between mb-2">
                        ...
                        <div class="col-sm-8">
                            <div class="mb-2 text-sm-end">
                                <a href="{{ url('leave/manage/leave-type-property/create') }}" class="btn btn-primary waves-effect waves-light">
                                    <i class="mdi mdi-plus-circle me-1"></i> เพิ่มจำนวนวันหยุด </a>
                            </div>
                        </div>
                    </div>
                    <table id="table" data-toggle="table" data-search="true" data-search-align="left" data-search-selector="#search" data-ajax="ajaxRequest" data-query-params="queryParams" class="table text-nowrap">
                        <thead>
                            <tr>
                                ...
                                <th data-field="name" data-sortable="true">ชื่อวันหยุด</th>
                                <th data-field="day" data-sortable="true">จำนวนวันหยุด</th>
...
<script type="text/javascript">
    ...
    function ajaxRequest(params) {
        var url = "{{ url('leave/manage/leave-type-property/search') }}";
        $.get(url + '?' + $.param(params.data)).then(function (res) {
            params.success(res)
        });
    }
    function deleteLeaveTypePropertyConfirmation(id) {
        Swal.fire({
            icon: "warning",
            title: "คุณต้องการลบจำนวนวันหยุด ใช่ไหม?",
            ...
            preConfirm: () => {
                return fetch(`/leave/manage/leave-type-property/del/` + id)
                    .then((response) => {
                        if (!response.ok) {
