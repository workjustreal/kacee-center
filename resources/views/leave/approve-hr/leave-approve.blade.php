@extends('layouts.master-layout', ['page_title' => "อนุมัติการลาโดยบุคคล"])
@section('css')
<!-- third party css -->
            ...
            <div class="page-title-box">
                <div class="page-title-right">
                    <ol class="breadcrumb m-0">
                        ...
                        <li class="breadcrumb-item active">อนุมัติการลาโดยบุคคล</li>
                    </ol>
                </div>
                <h4 class="page-title">อนุมัติการลาโดยบุคคล @if ($leave_count > 0)({{ $leave_count }}) @endif</h4>
            </div>
        </div>
    </div>
    <!-- end page title -->
            ...
            <div class="card">
                <div class="card-body">
                    <div class="row mb-2">
                        ...
                        <div class="col-md-auto col-sm-12">
                            <label for="period_salary" class="form-label">งวดค่าแรง</label>
                            <div class="form-group">
                                <select class="form-select" id="period_salary" name="period_salary">
                    ...
                    <table id="table" data-toggle="table" data-height="500" data-ajax="ajaxRequest" data-query-params="queryParams" data-search="true" data-search-selector="#search" data-custom-search="customSearch" class="table text-nowrap">
                        <thead>
                            <tr>
                                ...
                                <th data-field="leave_status" data-visible="false">STATUS</th>
                                ...
                                <th data-field="leave_status_text" data-sortable="true">สถานะ</th>
                                <th data-field="leave_manage">จัดการ</th>
                            </tr>
                        </thead>
                    </table>
...
<script type="text/javascript">
    ...
    function ajaxRequest(params) {
        var url = "{{ url('leave/approve-hr/leave-approve/search') }}";
    ...
    function approveHRLeaveConfirmation() {
        Swal.fire({
            icon: "warning",
            title: "ยืนยันอนุมัติการลางาน ใช่ไหม?",
            ...
            preConfirm: () => {
                return fetch('/leave/approve-hr/leave-approve/submit', {
                        method: 'POST',
                        headers: {
                            'Content-type': 'application/json; charset=UTF-8',
                            'X-CSRF-TOKEN': '{{csrf_token()}}',
                        },
                        body: JSON.stringify({'leave_id': objData}),
                    })
            ...
            if (result.isConfirmed) {
                Swal.fire({
                    icon: "success",
                    title: "อนุมัติการลางานเรียบร้อย!",
                    timer: 2000,
                });
                objData.shift();
