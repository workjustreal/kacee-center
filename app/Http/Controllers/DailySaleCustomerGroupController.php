...
class DailySaleCustomerGroupController extends DailySaleBaseController
{
    ...
    public function index()
    {
        ...
        return view('sales-report.daily-sales-customer', compact('daily_category', 'available_date', 'thead'));
    }
    ...
    public function search(Request $request)
    {
        if ($request->ajax()) {
            ...
            $items = self::callDataGroup($daily_category, $doc_date_start, $doc_date_end);
    ...
    public function print(Request $request)
    {
        ...
        if (count($items) <= 0) {
            ...
            $data["title"] = "ตารางสรุป วันที่ ".str_replace("/", "-", $date_short);
            $data["header"]["title"] = "ตารางสรุป วันที่ ".$date_short;
            $data["thead"] = $thead;
            $data["rowlist"] = $rowlist;
            $data["rows"] = $rows;

            return view('sales-report.daily-sales-customer-print', compact('data'));
        }
    }
    ...
    public function export(Request $request)
    {
        ...
        if (count($items) <= 0) {
            ...
            return Excel::download(new DailySalesCustomerExport($result, "ตารางสรุปยอด"), 'ตารางสรุป วันที่ '.str_replace("/", "-", $date_short).'.xlsx');
        }
    }

    public function callDataGroup($daily_category, $doc_date_start, $doc_date_end)
    {
        $daily_category = (!empty($daily_category)) ? $daily_category : "";
        $category = self::callDataGroupCat($daily_category, $doc_date_start, $doc_date_end);
        $level0 = self::callCustomerLevel(0);
        $level1 = self::callCustomerLevel(1);

        $summary = self::callGroupSummary($daily_category, $doc_date_start, $doc_date_end);
        ...
        foreach ($category as $cat) {
            ...
            if (in_array($_category, $this->fix_category)) {
                // ################################# FIX CATEGORY ####################################
                $fix_category = self::callDataFixGroupCat($_category, $doc_date_start, $doc_date_end);
                foreach ($fix_category as $fix_cat) {
                    $summary_unit = self::callGroupUnitSummary($_category, $doc_date_start, $doc_date_end, $fix_cat->unit);
    ...
    public function callDataGroupCat($daily_category, $doc_date_start, $doc_date_end)
    {
        $daily_category = (!empty($daily_category)) ? $daily_category : "";
        $sort = self::sortCategory("d");
        $items = DB::table('daily_sales as d')->leftJoin('ex_customer_parent as p', 'd.cuscod', 'like', DB::Raw("REPLACE(p.cuscod,'*','%')"))->where('d.stkcod', '<>', '');
    ...
    public function callDataFixGroupCat($daily_category, $doc_date_start, $doc_date_end)
    {
        $daily_category = (!empty($daily_category)) ? $daily_category : "";
        $sort = self::sortCategory("d");
        $items = DB::table('daily_sales as d')->leftJoin('ex_customer_parent as p', 'd.cuscod', 'like', DB::Raw("REPLACE(p.cuscod,'*','%')"))->where('d.stkcod', '<>', '');
