...
class NotificationSpecialDiscount extends Controller
{
    public function index(Request $request)
    {
        if ($request->ajax()) {
            $ntSpecial_mn = 0;
            $ntSpecial_sec = 0;
            $ntSpecial_personal = 0;
            $ntSpecial_secaction = 0;
            ...
            if (Auth::user()->noti_CheckApproveMar()) {
                ...
                $ntSpecial_mn = $data_mn->select('count(*) as allcount')->count();
            }
            if (Auth::user()->noti_CheckSecretary()) {
                $data_sec = Special::where('doc_status', '=', 1);
                $ntSpecial_sec = $data_sec->select('count(*) as allcount')->count();
            }
            ...
            if (Auth::User()->noti_CheckMar() && !is_int($admin_manager)) {
                ...
                $sec_action = Special::from('request_special_discount as r')
                    ->leftJoin('request_special_noti as n', 'n.doc_id', '=', 'r.doc_id')
                    ->where('r.emp_id', '=', auth()->user()->emp_id)
                ...
                $ntSpecial_personal = $personal->select('count(r.*) as allcount')->count();
                $ntSpecial_secaction = $sec_action->select('count(r.*) as allcount')->count();
            }

            $response = array(
                "ntSpecial_mn" => $ntSpecial_mn,
                "ntSpecial_sec" => $ntSpecial_sec,
                "ntSpecial_personal" => $ntSpecial_personal,
                "ntSpecial_secaction" => $ntSpecial_secaction,
            );
