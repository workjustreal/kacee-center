...
use App\Models\RequestDiscountLog as Log;
...
class ProductDiscountController extends Controller
{
    ...
    //     if ($date == "") {
    //         $dis_re = DiscountRepair::from('request_discount_repair as r')->leftJoin('request_discount_log as l', 'l.doc_id', '=', 'r.doc_id')
    ...
    //         // $dis_re = DiscountRepair::where('emp_id', '=', auth()->user()->emp_id)->where('doc_status', 'LIKE', $doc_status . '%')->whereBetween('created_at', [$start_date, $end_date])->get();
    //         $dis_re = DiscountRepair::from('request_discount_repair as r')->leftJoin('request_discount_log as l', 'l.doc_id', '=', 'r.doc_id')
    //             ->where('r.area_code', '=', $area->area_code)
    //             ->where('r.doc_status', 'LIKE', $doc_status . '%')
    ...
    //     }
    //     $request->flash();
    //     return view('sales-document.product-discount.list-personal', compact('dis_re'));
    // }
    public function index(Request $request)
    {
        $date = $request->doc_date;
        $doc_status = $request->doc_status;
        $area = Employee::where('emp_id', auth()->user()->emp_id)->value('area_code');

        $dis_re = DiscountRepair::from('request_discount_repair as r')
            ->leftJoin('request_discount_log as l', 'l.doc_id', '=', 'r.doc_id')
            ->where('r.area_code', $area)
            ->where('r.doc_status', 'LIKE', $doc_status . '%');
        ...
        if ($date !== null) {
            ...
            $start_date = date($_date[1] . "-" . $_date[0] . "-01 00:00:00");
            $end_date = date("Y-m-t", strtotime($start_date)) . " 23:59:59";
            $dis_re->whereBetween('r.created_at', [$start_date, $end_date]);
        }

        $dis_re = $dis_re->orderBy('r.created_at', 'DESC')
            ->groupBy('r.doc_id')
            ->selectRaw('r.*,
        CONCAT(
            "[",
    ...
    public function request(Request $request)
    {
        ...
        return view('sales-document.product-discount.request');
    }
    ...
    public function request_create(Request $request)
    {
        ...
        $gen = "DCR" . date("ym");
        $rundoc = DiscountRepair::whereRaw('SUBSTRING(doc_id, 1, 7) = "' . $gen . '"')->orderBy('created_at', 'DESC')->first('doc_id');
        if ($rundoc) {
        ...
        if ($customer->isNotEmpty() && $request->customer_name != '') {
            ...
            $dis_re->product_list = $request->product_list;
            $dis_re->invoice = $request->invoice;
            $dis_re->customer_request = $request->customer_request;
            $dis_re->mistake = $request->mistake;
            $dis_re->note = $request->note;
            $dis_re->image = json_encode($img_name, JSON_FORCE_OBJECT);
            $dis_re->description = $request->description;
            $dis_re->file = $file;
            $dis_re->area_code = $area->area_code;

            //check manager
            if ($_level->level == "1" || $_level->level == "2") {
                if ($_level->dept_id == "A01000000") {
                    $dis_re->doc_status = 9;
                    $dis_re->sec_approve = auth()->user()->emp_id;
                    $dis_re->sec_approve_date = now();
                    $dis_re->mn_approve = auth()->user()->emp_id;
                    $dis_re->mn_approve_date = now();
                    $dis_re->save();

                    $des_c = ["สร้างคำขอ", "ManagerApprove", "SecretaryApprove"];
                    foreach ($des_c as $des) {
                        $log = new Log();
                        $log->doc_id = $doc_id;
            ...
            return redirect('/sales-document/discount-mistake/productdiscount-list_personal');
        }else{
            alert()->error('กรุณาตรวจสอบคำขอ!');
            return redirect('/sales-document/discount-mistake/productdiscount-request');
        }
    }
    ...
    public function getEdit(Request $request, $doc_id, $id, $status)
    {
        ...
        $noedit = "";
        $img = "";
        $result = DiscountRepair::where('doc_id', '=', $doc_id)->get();
        $images = DiscountRepair::where('doc_id', '=', $doc_id)->where('image', '<>', 'null')->first('image');
        $file = DiscountRepair::where('doc_id', '=', $doc_id)->where('file', '<>', '')->first('file');
        $log = Log::from('request_discount_log as r')->leftJoin('employee as e', 'e.emp_id', '=', 'r.emp_id')
        ...
        foreach ($log as $chkedit) {
            if ($chkedit->description == 'แก้ไขเพื่อขออีกครั้ง') {
                if ($result[0]->doc_status == 3 || $result[0]->doc_status == 0) {
                    $noedit = "true";
                }
            }
        }
        if ($noedit) {
            ...
            return view('sales-document.product-discount.edit-request', compact('result', 'status', 'log', 'doc_id', 'img', 'file'));
        }
    }
    ...
    public function updateRequest(Request $request, $doc_id, $id, $status)
    {
        ...
        if (session()->has('request_img')) {
            if ($old_img) {
                $destinationPath = $_SERVER['DOCUMENT_ROOT'] . '/assets/images/discount/';
                // delete imgage
            ...
            $img_name = session()->get('request_img');
        } else {
            $img_name = '';
        }
        ...
        if ($customer->isNotEmpty()) {
            $discount = DiscountRepair::find($id);
            $discount->customer_code = $request->customer_code;
            $discount->customer_name = $request->customer_name;
            $discount->customer_status = $request->customer_status;
            $discount->limit = $request->limit;
            $discount->employee = $request->staf;
            $discount->product_list = $request->product_list;
            $discount->invoice = $request->invoice;
            $discount->customer_request = $request->customer_request;
            $discount->mistake = $request->mistake;
            $discount->note = $request->note;
            if ($img_name != '') {
                $discount->image = json_encode($img_name, JSON_FORCE_OBJECT);
            }
            if ($request->file) {
                $discount->file = $file;
            }
    ...
    {
        $request->session()->forget('request_img');
        $wimage = DiscountRepair::where('doc_id', '=', $doc_id)->where('image', '<>', 'null')->get('image');
        $wfile = DiscountRepair::where('doc_id', '=', $doc_id)->where('file', '<>', '')->get('file');
        $admin_manager = self::adminManager(auth()->user()->emp_id);
        $result = DiscountRepair::from('request_discount_repair as r')
        ...
        if ($result->isNotEmpty() && $result[0]->doc_status == $status) {
            foreach ($result as $val) {
                ...
                $log = Log::from('request_discount_log as l')->leftJoin('employee as e', 'e.emp_id', '=', 'l.emp_id')->where('l.doc_id', '=', $doc_id)
                    ->get([
                        'l.emp_id',
                        'l.description',
                        'l.comment',
                ...
                if ($val->mn_approve == "") {
                    ...
                    $mn_app = Employee::where('emp_id', '=', $val->mn_approve)->first(['name', 'surname', 'nickname']);
            ...
            return view('sales-document.product-discount.preview', compact('result', 'mn_approve', 'mn_dot', 'secretary_dot', 'st_approve', 'succes_dot', 'mn_date', 'st_date', 'status',  'mn_app', 'mn_date', 'log', 'sec_app', 'sec_date_log', 'mn_date_log', 'wimage', 'wfile', 'admin_manager', 'name_secapp'));
        } else {
            return back();
        }
    }
    ...
    {
        $wimg = DiscountRepair::where('id', '=', $request->id)->where('image', '<>', 'null')->first('image');
        $wfile = DiscountRepair::where('id', '=', $request->id)->where('file', '<>', '')->first('file');
        if ($wimg) {
        ...
        DiscountRepair::where('id', '=', $request->id)->delete();
    ...
    public function approveManager(Request $request)
    {
        ...
        if ($date == "") {
            if ($admin == "A0110") {
                $discount = DiscountRepair::from('request_discount_repair as r')->leftJoin('request_discount_log as l', 'l.doc_id', '=', 'r.doc_id')
                    ->where('r.doc_status', 'LIKE', $doc_status . '%')
                    ->orderBy('r.created_at', 'DESC')
                    ->groupBy('r.doc_id')
                    ->selectRaw('r.*,l.updated_at as printDate,
                    CONCAT(
                    "[",
                    GROUP_CONCAT(
                        JSON_OBJECT(
        ...
        $request->flash();
        return view('sales-document.product-discount.approve-manager', compact('discount', 'wapprove', 'admin_manager'));
    }

    // view list approve
    public function approveSecretary(Request $request)
    {
        $_status = ['0', '1', '9'];
        $date = $request->doc_date;
        $doc_status = $request->doc_status;
        ...
        $request->flash();
        return view('sales-document.product-discount.approve-secretary', compact('discount', 'wapprove'));
    }
    ...
    public function wApprove($position)
    {
        ...
        if ($position == "mn") {
            if ($admin == "A0110") {
                $req = DiscountRepair::where('doc_status', '=', 2)->orderBy('updated_at', 'DESC')
                    ->get();
            } else {
                $_dept = self::deptFormat(auth()->user()->dept_id);
                $req = DiscountRepair::where('doc_status', '=', 2)
                    // ->where('dept_id', 'LIKE', $_dept . '%')
                    ->where(function ($query) use ($admin, $_dept) {
                        if ($admin == 'A0202' || $admin == 'A0201') {
            ...
            $req = DiscountRepair::where('doc_status', '=', 1)->orderBy('updated_at', 'DESC')
                ->get();
        }
        return $req;
    }
    ...
    public function secretaryApprove(Request $request)
    {
        if ($request->approve == "yes") {
            $discount = DiscountRepair::find($request->id);
            $discount->doc_status = 9;
            $discount->sec_approve = auth()->user()->emp_id;
            $discount->sec_approve_date = now();
            $discount->updated_at = now();
            $discount->update();
    ...
    public function previewManagerApprove(Request $request, $doc_id, $id)
    {
        ...
        if ($request->approve == "yes") {
            ...
            $discount->updated_at = now();
            if ($img_name == '') {
                $discount->image_mn = $img_name;
            } else {
                $discount->image_mn = json_encode($img_name, JSON_FORCE_OBJECT);
            }
            $discount->file_mn = $file;
            $discount->update();
            $log_des = "ManagerApprove";
            $log_comment = $request->comment;
            self::saveLog($doc_id, $log_des, $log_comment);
            return redirect('sales-document/discount-mistake/manager-approve/product-discount-repair');
        } else {
            $discount = DiscountRepair::find($id);
    ...
    // search auto
    public function createAutoSearch(Request $request)
    {
    ...
    {
        // $data = DiscountRepair::where('id', '=', $id)->get();
        $data = DiscountRepair::from('request_discount_repair as r')->leftJoin('request_discount_log as l', 'l.doc_id', '=', 'r.doc_id')
            ->where('r.doc_id', '=', $doc_id)
            ->groupBy('r.doc_id')
            ->selectRaw('r.*,
                    CONCAT(
        ...
        $dompdf->loadView('sales-document.product-discount.pdf.pdf-discount-repair', compact('data'))
            ->setPaper('a4')
            ->setWarnings(false);
    ...
    // upload image requestdiscount
    public function imageRequest(Request $request)
    {
        $image = $request->file('file');
        if ($image) {
            $filename = $image->getClientOriginalName();
            session()->push('request_img',  $filename);
            // $filename = date("Ymdhis") . '_' . $i . '.' . $image->extension();
            $destinationPath = $_SERVER['DOCUMENT_ROOT'] . '/assets/images/discount/';
    ...
    {
        $_img = session()->pull('request_img', []); // Second argument is a default value
        if (($key = array_search($request->name, $_img)) !== false) {
            unset($_img[$key]);
        }
        session()->put('request_img', $_img);
    ...
    public function report(Request $request)
    {
        ...
        $request->flash();
        return view('sales-document.product-discount.report', compact('dis_re'));
    }
